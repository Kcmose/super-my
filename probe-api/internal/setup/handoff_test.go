...
func TestLoadInstalledAccessReconstructsStrictDomainAndIPAdminURLs(t *testing.T) {
	for name, environment := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			if name == "ip" && (access.Mode != IngressModeIP || access.AdminURL != "https://[2001:db8::20]:18455/login") {
				t.Fatalf("IP access = %#v", access)
			}
			if name == "domain" && (access.Mode != IngressModeDomain || access.AdminURL != "https://admin.monitor.test/login") {
