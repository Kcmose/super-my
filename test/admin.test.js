...
test('one-time Agent credentials are never persisted or logged by the component', async () => {
  ...
  assert.match(source, /response\?\.install_command/)
	assert.match(source, /response\?\.agent\?\.status/)
	assert.match(source, /Agent 接入尚未配置/)
	assert.match(source, /!payload\.enabled \|\| !agentRuntimeEnabled\.value/)
	assert.match(source, /error\?\.code === 'agent_not_configured'/)
  assert.match(source, /Shell 历史/)
	assert.match(source, /可使用 sudo/)
	assert.match(source, /进程参数/)
